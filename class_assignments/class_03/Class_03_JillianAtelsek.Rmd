...
#Columns stored as "dbl" that don't need to be include "BUYER_ZIP," "REPORTER_ZIP," "TRANSACTION_ID" and "DRUG_CODE." Maybe the date should be stored as another format so that we could sort by it easier. 
...
# BUYER_ZIP represents the zip code of the pharmacy where the pills were sent. The mean value is 25594, but that's not a meaningful thing for us to use because zip codes aren't a mathematical value or anything that we would need to perform operations on.
# DOSAGE_UNIT represents the amount of pills that were sent in a certain transaction. The minimum value is 10, the maximum value is 25,000, and the mean value is 697.5. These are meaningful statistics because they give us an overview of what transactions into Logan County looked like and they're based in values that it makes sense to perform operations on. 
...
# Select all the data but the first four columns. 
...
# Look only at the columns that include the term "buyer."
...
# Remove three objects.

rm(logan_pipeline, logan_all_but_four, logan_buyer)

# Remove all but "logan": You can turn the "environment" section into a grid and check all but one box to clear them. Or:
...
logan_pipeline_sorted <- select(logan, REPORTER_NAME, BUYER_NAME, Combined_Labeler_Name, DOSAGE_UNIT)

arrange(logan_pipeline_sorted, Combined_Labeler_Name)
...
# Creating a function to display the manufacturer/distributor/pharmacy pipleine, along with dosage units, and sort it in reverse alphabetical order by pharmacy name. 

logan_pipeline_sorted <- logan %>% 
  select(REPORTER_NAME, BUYER_NAME, Combined_Labeler_Name, DOSAGE_UNIT) %>%
