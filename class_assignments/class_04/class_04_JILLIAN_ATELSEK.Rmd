...
#The data has 42 columns and 290,298 rows.

#Columns that are stored as "dbl" that don't need to be include REPORTER_ZIP, DRUG_CODE and TRANSACTION_ID. There's a chance it could be useful to store the date in numerical format, but I kind of doubt it matters. 

#Missing values are represented by the word "null."
...
# The DEA and Maryland sued Newcare, and it was convicted for 2008 of selling almost 10 million units of hydrocodone illegally over the Internet to anyone with a valid credit card. They were paid for each perscription they filled by a small group of doctors.
...
# The distributor was McKesson Corporation. They sent 58,267,330 pills to Baltimore City, and they're headquartered in Texas
...
# The name is PROFESSIONAL PHARMACY SERVICE, and it's headquartered in Sinai Hospital at 2411 WEST BELVEDERE AVENUE. It's unique that this pharmacy is within a hospital, which might explain why they got so many shipments. 	
  ...
  group_by(buyer_name) %>%
   summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
    mutate(total_pills = sum(pills_per_pharmacy)) %>%
      mutate(pills_percent = round((pills_per_pharmacy/total_pills)*100,2)) %>%
        arrange(desc(pills_percent))
      
# The pharmacy that got the highest percentage of pills was RITE AID OF MARYLAND, INC. 12.52 percent of all pills shipped to Baltimore City were shipped to them. They received 16,740,590 pills, while the city as a whole received 133,721,503.	
...
baltimore %>%
  group_by(buyer_name, buyer_bus_act) %>%
  summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
  mutate(total_pills = sum(pills_per_pharmacy)) %>%
  mutate(pills_percent = pills_per_pharmacy/total_pills) %>%
  arrange(desc(pills_percent)) %>%
  filter(str_detect(buyer_name, "RITE"))

# There are three different iterations of Rite Aid in this data set. They weren't grouped correctly because the names differ very slightly: One has a commma and a period, one has neither, and one has a comma and "T/A" at the end.
...
# There were 290908 shipments to Baltimore City, and 15.1 percent of them went to Rite Aid stores.
...
# The pipeline that handled the most shipments and the most pills was SpecGx LLC--RITE AID MID-ATLANTIC--RITE AID OF MARYLAND, INC. The pipeline with the highest number of pills handled per shipment, on average, was Actavis Pharma, Inc.--CARDINAL HEALTH--NEWCARE HOME HEALTH SERVICES. I would look at all of these entities to spur additional reporting and see why their numbers were so high.
...
baltimore %>%
  ...
  select(transaction_date, dosage_unit)
...
# Q: Who made most of the pills Newcare got?
...
# Q: What percent of pills sent to Newcare came from Actavis?
...
# Q: Which distributor sent the most pills to Newcare?
...
# A: Anda Pharmaceuticals Inc. 

```

```{r}

# How many pills per shipment did Newcare get on average?
