...
buyer_annual <- read.csv("data/buyer_annual_by_year.csv")

state_pop <- read.csv("data/state_population_per_year.csv")
...
# The buyer_annual has the most data because it contains values for each year from 2006-2012, whereas the other two summarize the data over the whole six-year period. I think buyer_addresses is higher than buyer_totals because some pharmacies have more than one physical location, giving them different addresses.
...
# The pharmacy that had the most pills sent to it was VA CONSOLIDATED MAIL OUTPATIENT PHARMACY in Charleston, South Carolina. It's located at 3725 RIVERS AVENUE. It had so many pills sent to it because it is a mail-order pharmacy run by the VA. It delivers pills to veterans across the Southeast, and is one of only seven such facilities in the nation.
...
# RAKESH K MATHUR is the practitioner who got the most total pills in 2010. The doctor had 337,450 pills sent to him, whereas the next highest-ranking doctor received 212,800. Mathur was banned from distributing medication in 2012 after the board found that his actions constituted "a risk of serious harm to the public health, safety and welfare based on the physician’s prescribing of opiod medications while failing to use reasonable safeguards or to exercise due diligence." To ensure that the doctors are the same, I would
...
pills_per_capita <- totals_per_state %>%
  ...
  mutate(pills_per_capita = total_pills/population_average)
  
# West Virginia had the most pills per person sent to it between 2006 and 2012. 
...
# join buyer addresses with buyer totals by dea number, and return only records for which there's a total match
buyer_totals %>%
  inner_join(buyer_addresses, by="buyer_dea_no")

# join the the two sets by dea number, but return only records for which there is a match in the buyer_totals field
buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no")

# same as above, but only show records where the address field is empty
buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no") %>%
  filter(buyer_address1 =="na")
```
