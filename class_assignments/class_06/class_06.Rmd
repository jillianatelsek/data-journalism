...
state_population(2006)

buyer_details(county="Frederick", state="MD", key=key)	

summarized_county_monthly(county="Frederick", state="MD", key=key)	
...
Maryland <- buyer_totals %>%
 filter(buyer_state=="MD")

Frederick_details <- buyer_details(county = "Frederick", state="MD", key=key)

Frederick_details <- clean_names(Frederick_details)

Maryland <- Maryland %>%
  inner_join(Frederick_details, by="buyer_dea_no")
 
Walkersville <- Maryland %>%
  filter(buyer_city=="WALKERSVILLE") %>%
