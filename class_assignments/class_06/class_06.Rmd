...
PG_annual <- combined_buyer_annual(county="Prince Georges", state="MD", key=key)

PG_details <-buyer_details(county = "Prince Georges", state="MD", key=key)

PG_complete <- PG_annual %>%
  inner_join(PG_details, by="BUYER_DEA_NO")
