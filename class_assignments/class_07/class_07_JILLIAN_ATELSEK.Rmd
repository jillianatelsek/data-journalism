...
logan_pills_per_year <- arcos_county_pills_per_year %>%
 ...
 ggplot(logan_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="Over 6 million pills were sent to Logan County every year from 2006 and 2012", subtitle = "Total pills shipped to Logan County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

  # Also saves a web version
  ggsave("logan_pills.png", plot=last_plot())
  
  #Findings: The pills sent to Logan County stayed relatively high over the six-year period. More than 6 million pills were sent to the county every year over this period, with the peak year being 2009.
...
wv_counties_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", (buyer_county %in% c("KANAWHA", "CABELL", "MINGO", "RALEIGH","LOGAN", "WOOD"))) %>%
  select(buyer_county, year, dosage_unit)

ggplot(wv_counties_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit, fill=buyer_county)) +
  labs(x="Year", y="Total pills", title="Steady rise in opioids in Kanawha and Raleigh counties", subtitle = "Total pills per year shipped to Kanawha, Cabell, Mingo, Raleigh, Logan and Wood counties", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
...
#Findings: Kanawha county received more pills per year than the other five counties. Kanawha and Raleigh counties saw the steadiest increase in pills per year between 2006 and 2012. Mingo County saw a dramatic spike in 2008, followed be a sharp decrease. The other counties remained relatively stable across the period. 
...
md_counties_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD", year=="2012") %>%
  select(buyer_county, year, dosage_unit, countyfips)

md_counties_population_2012 <-county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "MD", year=="2012") %>%
  select(buyer_county, population, countyfips)
  
md_pills_and_pop_2012 <- md_counties_pills_2012 %>%
    inner_join(md_counties_population_2012, by=("countyfips"))

md_pills_and_pop_2012 <- md_pills_and_pop_2012 %>%
   select(buyer_county.x, countyfips, year, dosage_unit, population)


md_pills_and_pop_2012 <- 
  mutate(md_pills_and_pop_2012, pills_per_person = dosage_unit/population)


md_2012 <-
md_pills_and_pop_2012 %>%
 inner_join(md_county_median_household_income, by = c("countyfips" = "GEOID"))


md_2012 <-
  md_2012 %>%
  select(buyer_county.x, countyfips, year, dosage_unit, population, estimate)
...
ggplot(md_2012) +
  ...
  labs(x="Median household income", y="Opioid pills per capita", title="Poorer Maryland counties got more pills per person in 2012", caption = "Source: DEA ARCOS database, via Washington Post") +
  ...
  geom_text_repel(aes(estimate, pills_per_person, label=buyer_county.x), 
    subset(md_2012, pills_per_person > 40))
...
last_question <- arcos_county_pills_per_year %>%
...
```{r}
md_counties_population <-county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "MD") %>%
  select(buyer_county, year, population, countyfips)

```
  
```{r}

last_question_graphic <- last_question %>%
   full_join(md_counties_population)


last_question_graphic <- last_question_graphic %>%
  mutate(pills_per_person = dosage_unit/population)
```

```{r}

last_question_graphic <-
  last_question_graphic %>%
  filter(buyer_county %in% c("KENT", "ALLEGANY", "WASHINGTON", "WICOMICO", "WORCESTER", "HARFORD"))
...
ggplot(last_question_graphic) +
  geom_bar(stat="identity", aes(year, pills_per_person, fill=buyer_county)) +
  labs(x="Year", y="Total pills per capita", title="Kent and Allegany counties received most pills per person", subtitle = "Total pills per capita per year shipped to Allegany, Harford, Kent, \nWashington, Wicomico and Worcester counties", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
  facet_wrap(nrow=2, . ~ buyer_county) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
