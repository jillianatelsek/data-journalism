...
# use the clean_names function in janitor 
...
mingo %>%
  ...
  summarise(pills_per_combo = sum(dosage_unit)) %>%
  arrange(desc(pills_per_combo))    

#The most significant combination is Actavis, Miami-Luken and Tug Valley Pharmacy.
...
mingo %>%
  group_by(combined_labeler_name, reporter_name, reporter_dea_no, reporter_city, reporter_address1,
buyer_name) %>%
  summarise(pills_per_combo = sum(dosage_unit)) %>%
...
##  I would use one of the functions we learned above that lets you combine or merge data values. I would tell R to merge the data for each value in the reporter_name column that contains the words "CARDINAL HEALTH," and make a new reporter_name that includes all of their locations. If I discovered this was a problem with other distributors as well, I would repeat the same solution. The easiest way to do this might be with the case_when function, by telling R to isolate every entry for which the reporter_name column includes "CARDINAL HEALTH." 
